...
const MiniCard = ({ time, temp, iconString, degrees }) => {
  ...
  useEffect(() => {
    if (iconString) {
      ...
      setIcon(iconMap[matchedIcon] || null)
    }
  }, [iconString])

  const formattedTime = new Date(time).toLocaleTimeString('en', { weekday: 'long' }).split(" ")[0]
  const temperature = degrees === 'F' ? ((temp * 9/5) + 32).toFixed(1) : temp
  ...
  return (
    <div className='glassCard w-[10rem] h-[10rem] p-4 flex flex-col'>
      ...
      <p className='text-center font-bold'>{temperature} &deg;{degrees}</p>
    </div>
  )
}
